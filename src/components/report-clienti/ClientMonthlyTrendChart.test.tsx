...
// Mock useClientMonthlyTrend hook
...
const mockUseClientMonthlyTrend = vi.fn(
  (fiscalYear: number, clientId: string | null | undefined) => {
...
vi.mock("@/hooks/useClientMonthlyTrend", () => ({
  useClientMonthlyTrend: (...args: [number, string | null | undefined]) => mockUseClientMonthlyTrend(...args),
}));

describe("ClientMonthlyTrendChart", () => {
  ...
  it("renders only months with data (sparse)", () => {
    ...
    // 3 months with data → 3 entries in chart
  ...
  it("shows previous year bar when YoY toggle is activated", () => {
    ...
    // Now previous bar should appear
  ...
  it("renders loading skeleton when data is loading", () => {
    mockUseClientMonthlyTrend.mockReturnValueOnce({
      data: undefined,
      isLoading: true,
      error: null,
    });
  ...
  it("handles null clientId (Senza cliente)", () => {
    render(
      <ClientMonthlyTrendChart
        clientId={null}
        clientName="Senza cliente"
        fiscalYear={2026}
      />,
    );
    expect(screen.getByTestId("bar-chart")).toBeInTheDocument();
