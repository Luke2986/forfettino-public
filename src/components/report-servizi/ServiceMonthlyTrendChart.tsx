...
import { useServiceMonthlyTrend, type ServiceMonthlyData } from "@/hooks/useServiceMonthlyTrend";
...
interface ServiceMonthlyTrendChartProps {
...
export function buildTrendData(
  currentData: ServiceMonthlyData[],
  previousData: ServiceMonthlyData[] | undefined,
): TrendBarData[] {
...
export function ServiceMonthlyTrendChart({
  serviceId,
  serviceName,
  fiscalYear,
}: ServiceMonthlyTrendChartProps) {
  const [showPreviousYear, setShowPreviousYear] = useState(false);

  const { data: currentData, isLoading: currentLoading } =
    useServiceMonthlyTrend(fiscalYear, serviceId);

  const { data: previousData, isLoading: previousLoading } =
    useServiceMonthlyTrend(
      fiscalYear - 1,
      showPreviousYear ? serviceId : undefined,
    );
  ...
  if (!currentData?.length && !previousData?.length) {
    return (
      <div className="flex items-center justify-center h-[200px] text-sm text-slate-500">
        Nessun dato disponibile per {serviceName}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        <Switch
          id={`yoy-service-toggle-${serviceId ?? "null"}`}
          checked={showPreviousYear}
          onCheckedChange={setShowPreviousYear}
        />
        <Label
          htmlFor={`yoy-service-toggle-${serviceId ?? "null"}`}
