...
RETURNS TABLE (
  service_id UUID, service_name TEXT, service_color TEXT,
  total_gross NUMERIC, total_net NUMERIC, receipt_count INT,
  first_receipt_date DATE, last_receipt_date DATE, percentage NUMERIC
)
...
AS $$
  SELECT
    ...
    ROUND(100.0 * SUM(r.gross_amount) / NULLIF(SUM(SUM(r.gross_amount)) OVER (), 0), 2)::NUMERIC AS percentage
  FROM receipts r
