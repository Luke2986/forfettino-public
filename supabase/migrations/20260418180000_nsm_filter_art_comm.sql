-- ============================================================================
-- Hot-fix NSM: aggiunto valore 'art_comm' al filtro p_gestione_filter.
-- Data: 2026-04-18
--
-- Motivazione: il widget AdminNSMMiniCard split layout mostra due sezioni
-- affiancate (Gestione Separata | Artigiani + Commercianti). Le gestioni
-- artigiani e commercianti hanno scadenze IDENTICHE (inps_q1..q4 +
-- acconto_inps_1/2 + acconto_tax_1/2) quindi mostrarle separate non e'
-- diagnostico e moltiplicherebbe inutilmente le RPC call. Le aggreghiamo
-- via filtro server-side 'art_comm' invece che client-side.
--
-- Modifiche minimali vs 20260418150000_nsm_window_90gg_default.sql:
--   1. Validazione input: whitelist estesa con 'art_comm'.
--   2. Clausola filter su valid_users.inps_management: aggiunto ramo OR per
--      IN ('artigiani','commercianti') quando p_gestione_filter='art_comm'.
--
-- Il default p_window_days resta 90 (invariato). La firma funzione e'
-- invariata — chiamanti che passano 'separata', 'artigiani', 'commercianti'
-- o NULL non sono impattati.
--
-- Rollback: re-applicare 20260418150000_nsm_window_90gg_default.sql.
-- ============================================================================
...
BEGIN
  ...
  -- Whitelist estesa con 'art_comm' (aggregato artigiani + commercianti).
  ...
  valid_users AS (
    ...
    WHERE p.is_internal IS NOT TRUE
      ...
      -- Filtro gestione esteso con 'art_comm' (matcha artigiani OR commercianti).
      AND (
        p_gestione_filter IS NULL
        OR fys.inps_management = p_gestione_filter
        OR (p_gestione_filter = 'art_comm'
            AND fys.inps_management IN ('artigiani', 'commercianti'))
      )
  ),
