...
-- 1. ALTER TABLE profiles ADD COLUMN wizard_variant
-- 2. UPDATE RPC get_wizard_funnel con filtro variant
-- 3. UPDATE RPC get_ttv_dashboard con filtro variant
-- 4. NEW RPC get_ab_test_comparison con z-test significativita'

-- ============================================================
-- 1. Colonna profiles.wizard_variant
-- ============================================================
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS wizard_variant TEXT DEFAULT NULL;
-- Nessun indice dedicato, nessuna constraint CHECK (validazione client-side)
...
-- Drop old 3-param grant and add new 4-param grant
...
BEGIN
  ...
  WITH user_ttv AS (
    -- Per ogni utente qualificato, calcola TTV in minuti + data primo incasso
    SELECT
      p.user_id,
  ...
  all_signups AS (
    -- Conta tutti gli utenti con onboarding completato nel range (anche senza incassi)
  ...
  -- daily_trend: per ogni giorno negli ultimi p_trend_days, calcola median TTV
  -- degli utenti che hanno fatto il PRIMO incasso in quel giorno
  date_series AS (
...
BEGIN
  ...
  WITH variant_users AS (
    -- Assegna variant: NULL trattato come 'control'
    SELECT
      p.user_id,
  ...
  -- Utenti che hanno iniziato il wizard (almeno un wizard_step_entered)
  started AS (
  ...
  -- Utenti che hanno completato il wizard
  completed AS (
    SELECT DISTINCT vu.variant_name, e.user_id,
  ...
  -- Z-test per differenza completion rate (two-proportion z-test)
  z_test AS (
    SELECT
