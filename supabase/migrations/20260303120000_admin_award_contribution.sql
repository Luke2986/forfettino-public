-- Story 26.5: Admin Assegna Punti Manuale
-- - Nuovo action_type 'admin_manual'
-- - RPC admin_award_contribution (singolo/bulk, config/manuale)
-- - get_my_contributions aggiornata con admin_manual_pts

----------------------------------------------------------------------
-- 1. Aggiornare CHECK constraint per includere 'admin_manual'
----------------------------------------------------------------------
...
----------------------------------------------------------------------
-- 2. Seed 'admin_manual' in contribution_action_config
----------------------------------------------------------------------
...
----------------------------------------------------------------------
-- 3. RPC admin_award_contribution
----------------------------------------------------------------------
...
BEGIN
  ...
  IF p_action_type = 'admin_manual' THEN
    -- Manual: use provided points
    IF p_points IS NULL OR p_points <= 0 THEN
      RAISE EXCEPTION 'p_points obbligatorio e > 0 per admin_manual';
    END IF;
    v_points := p_points;
  ELSE
    -- Config-driven: read from contribution_action_config
  ...
  IF p_all_users THEN
    -- Bulk: all active non-internal users
    WITH inserted AS (
...
----------------------------------------------------------------------
-- 4. Aggiornare get_my_contributions — aggiungere admin_manual_pts
----------------------------------------------------------------------
