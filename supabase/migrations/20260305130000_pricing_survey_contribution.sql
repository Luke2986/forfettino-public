-- Pricing Survey (Van Westendorp) — contribution action + RPC
-- Depends on: contribution_action_config, user_contributions tables

----------------------------------------------------------------------
-- 1. Extend CHECK constraint to allow 'pricing_survey_completed'
----------------------------------------------------------------------
...
VALUES
  ('pricing_survey_completed', 10, 'Survey Pricing', 'una tantum', 'bg-violet-500', 'text-violet-700', 5)
ON CONFLICT (action_type) DO NOTHING;

----------------------------------------------------------------------
-- 3. RPC: record_pricing_survey_contribution (one-time, from config)
----------------------------------------------------------------------
...
BEGIN
  ...
  -- Verify user actually submitted the pricing survey
  IF NOT EXISTS (
...
GRANT EXECUTE ON FUNCTION public.record_pricing_survey_contribution() TO authenticated;
